    ...
    <form>
      ...
      <div className="grid grid-cols-2 sm:flex gap-4 my-2 flex-wrap">
        {Object.keys(images).map((key) => (
          <label htmlFor={`roomImage${key}`} key={key}>
            <img className="max-h-13 cursor-pointer opacity-80" src={images[key] ? URL.createObjectURL(images[key])
      ...
      <div className='w-full flex max-sm:flex-col sm:gap-4 mt-4'>
        <div className='flex-1 max-w-48'>
          ...
          className='border opacity-70 border-gray-300 mt-1 rounded-2xl p-2 w-full'>
            <option value="">Select Room Type</option>
            <option value="Single Bed">Single Bed</option>
            <option value="Double Bed">Double Bed</option>
            <option value="Luxury Bed">Luxury Room</option>
            <option value="Family Bed">Family Suit</option>
          </select>
        </div>
        <div>
            ...
            mt-1 rounded p-2 w-24' value={inputs.pricePerNith} 
            onClick={e=>setInputs({...inputs, pricePerNith: e.target.value})}/>
        </div>
      </div>

        ...
        create unforgettable memories.'/>
            ...
            <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-12'>
                {exclusiveOffers.map((items)=>(
                    <div key={items._id} className='group relative flex flex-col itmes-start justify-between
                    gap-1 pt-12 md:pt-18 rounded-xl text-white bg-no-repeat bg-cover bg-center' 
                    style={{backgroundImage:`url(${items.image})`}}>
                         ...
                         <div>
                            <p className='text-2xl font-medium font-Playfair pl-4'>{items.title}</p>
                            <p className='pl-4.5'>{items.description}</p>
                            <p className='text-xs text-white/70 mt-3 pl-3.5'>Expires{items.expiryDate}</p>
                         </div>
                         <button className='flex items-center gap-2 font-medium
                         coursor-pointer mt-4 mb-5 pl-3.5'>
                            View Offers
